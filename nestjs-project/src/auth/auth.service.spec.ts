...
interface QueryBuilderMock {
  update: jest.Mock<QueryBuilderMock, [unknown?]>;
  set: jest.Mock<QueryBuilderMock, [unknown?]>;
  where: jest.Mock<QueryBuilderMock, [unknown?, unknown?]>;
  andWhere: jest.Mock<QueryBuilderMock, [unknown?, unknown?]>;
  execute: jest.Mock<Promise<undefined>, []>;
}
...
type MockMailService = jest.Mocked<
  Pick<MailService, 'sendConfirmationEmail' | 'sendPasswordResetEmail'>
>;

type MockVerificationTokenRepository = jest.Mocked<
  Pick<
...
type MockRefreshTokenRepository = jest.Mocked<
  Pick<
...
function createQbMock(): QueryBuilderMock {
  const qbMock = {
    update: jest.fn(),
    set: jest.fn(),
    where: jest.fn(),
  ...
  qbMock.update.mockReturnValue(qbMock);
  qbMock.set.mockReturnValue(qbMock);
