...
describe('Auth (e2e)', () => {
  ...
  async function registerConfirmAndLogin(
    ...
    const res = await request(app.getHttpServer())
      .post('/auth/login')
      .send({ email, password });
    const body = res.body as AuthTokensResponseBody;
    return {
  ...
  describe('POST /auth/refresh', () => {
    ...
    it('returns 200 with valid access token when reuse is within grace period', async () => {
      ...
      expect((res.body as AuthTokensResponseBody).access_token).toBeDefined();
  ...
  describe('POST /auth/reset-password', () => {
    it('returns 204 with a valid token and updates the password', async () => {
      ...
      const loginRes = await request(app.getHttpServer())
        ...
        .expect(200);
      expect(
        (loginRes.body as AuthTokensResponseBody).access_token,
      ).toBeDefined();
    });
